// ZipCodeAPI key
const zipKey =
  "CJjDASN6yTbqhEBYPoGi6tapGBm7wyq08Ilrn7OEiR1Ll0AnbONxYKXriUusT0uC";
// Array for all US states for our search bar
  const stateArr = [
  "AL",
  "AK",
  "AZ",
...
// Variable to create an object off of every search
let currentCity = {
  name: "Minneapolis",
  state: "",
  zipCode: "55401",
...
// Creates the stateArray dropdown after array is loaded.
window.onload = createDropDown;

// If statements to make sure we do a) notify of failed searches, and b) do not allow our search buttons to work without proper (or any) objects.
//PSEUDOCOPE ATTENTION: Add If statements here

// Clickhandlers for each of our four principle buttons.
$("#searchButton").click(() => {
  searchHandler(searchBar.val(), stateSelect.val());
});
...
$("#foodTab").click(() => {
  loadFood(currentCity);
});

$("#eventsTab").click(() => {
  loadEvent(currentCity);
});
...
function createDropDown() {
  for (x in stateArr) {
    stateSelect.append(
      `<option value='${stateArr[x]}'>${stateArr[x]}</option>`
    );
  }
}
...
function searchHandler(search, state) {
  searchBar.val("");
  let possibleNumber = parseInt(search);
...
function cityOnly(city, state) {
  let queryUrl = `https://www.zipcodeapi.com/rest/${zipKey}/city-zips.json/${city}/${state}`;
  ...
  $.ajax({
    ...
    .then(function(response) {
      let firstZipCode = response.zip_codes[0];
...
function zipHandler(zip) {
  let queryUrl = `https://www.zipcodeapi.com/rest/${zipKey}/info.json/${zip}/degrees`;
...
function callWeatherAPI(zip) {
  let queryUrl =
    ...
    zip +
    "&units=imperial&APPID=7c23f04d70a1191aa2f4e9bdcadb84b2";
  ...
  $.ajax({
    ...
    let weatherInfo = response.weather[0].description;
    // write the weather city name on the dome
    let weatherHeader = "<h1>Current weather Conditions for " + response.name;
    +"</h1>";
...
function callforcastAPI(zip) {
  queryUrl =
    ...
    zip +
    "&cnt=38&units=imperial&APPID=7c23f04d70a1191aa2f4e9bdcadb84b2";
  ...
  $.ajax({
    ...
    let forecastHeader = "<h1>5 day forecast for " + forecastdata.city.name;
    +"</h1>";
    ...
    for (let i in forecastInfo) {
      let currenttime = moment(forecastInfo[i].dt_txt).format(
        "ddd, MM-DD-YYYY, hA"
      );
      ...
      forecastDatahtml +=
        ...
        "</td>" +
        "<td>" +
        forecastInfo[i].main.humidity +
        "</td>" +
        "<td>" +
        forecastInfo[i].main.temp_min +
        "</td>" +
        "<td>" +
        forecastInfo[i].main.temp_max +
        "</td>" +
        "<td>" +
        forecastInfo[i].weather[0].description +
        "</td>" +
        "</tr>";
    }
    let forecastDiv = `
...
function foodSearchBar(cityObj) {
  let sfood = "resturants";
  let ratings = 3;
  let cost = "$$$$";

  $(".price").on("click", function() {
    cost = $(this).attr("data-content");
    let latitude = cityObj.coordinates[0];
    let longitude = cityObj.coordinates[1];
    let locate = `latitude=${latitude}&longitude=${longitude}`;
    const queryURL =
    ...
    $.ajax({
      ...
      headers: {
        Authorization:
          "Bearer vDe83QZUJVEgLPbqTMjeIQGWDB7NhgrxRsWNyC4GGUYEzUmwxFGyecJ61y-U0SAW6QTEpWhOBqZN9yBMpXnueUDldvGqnskcz3ydHuVB9V3wglA_ro_VMhAa-bNdXHYx"
      },
      method: "GET"
    }).then(function(response) {
      $("#content").empty();
      ...
      for (let i = 0; i < 20; i++) {
        ...
        if (resp.rating > ratings && cmount < viewa && resp.price <= cost) {
          ...
          let urll = resp.url;
          let address =
            resp.location.display_address[0] +
            " " +
            resp.location.display_address[1];
  ...
  $("#fsb").on("click", function() {
    sfood = $("#searchf")
      ...
      .trim();
    let latitude = cityObj.coordinates[0];
    let longitude = city.coordinates[1];
    let locate = `latitude=${latitude}&longitude=${longitude}`;
    const queryURL =
      "https://api.yelp.com/v3/businesses/search?term=" +
      sfood +
...
function eventAPICall(cityObj) {
  let where = cityObj.name;

  let queryURL =
    "https://api.eventful.com/json/events/search?app_key=Pts8nj75srVCqq6J&location=" +
    where +
    "&date=This+Week&page_size=5&sort_order=popularity";
  ...
  $.ajax({
    ...
    response.events.event.forEach(event => {
      ...
      let date = moment(event.start_time).format("dddd, MMMM Do YYYY, h:mm a");
